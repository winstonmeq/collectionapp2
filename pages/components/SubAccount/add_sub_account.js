...
const Add_sub_account = () => {
  ...
  const add_sub_account_db = async () => {
    try {
      ...
      const response = await axios.post(
        process.env.NEXTAUTH_URL + "/api/Sub_account/create_sub_account",
        payload
      );
  ...
  useEffect(() => {
    ...
    async function fetchData2() {
      const { data } = await axios.get(
        process.env.NEXTAUTH_URL + `/api/Account/get_account_sub`
      );
      setdatalist2(data);
    }
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <Button
          size={"sm"}
          onClick={(e) => {
            onOpen(), setAccount_id(row._id), setAccount_name(row.account_name);
          }}
        >
          {row.data2 ? row.account_name : ""}
        </Button>
      ),
      sortable: true,
    },
  ...
  return (
    <Flex direction={"column"} justify={"center"}>
          ...
          <ModalBody>
            ...
            <option value=''>Select</option>
                ...
                {datalist.map((items,i) => (
                  ...
                  <Box>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        setSub_account_name(e.target.value);
                      }}
                    />
                  </Box>
                  {/*                
              ...
              <label>Sub-Account Description</label>
                ...
                <Box>
                  <Button size="sm" onClick={add_sub_account_db} type="submit">
                    Save
                  </Button>
                </Box>
              </Stack>
      ...
      <DataTable columns={columns} data={datalist2} />
    </Flex>
  );
};
